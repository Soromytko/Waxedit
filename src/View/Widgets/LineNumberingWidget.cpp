...
static rendell_text::TextLayoutSharedPtr createTextLayout(std::wstring &&text,
                                                          glm::ivec2 fontSize) {
...
static rendell_text::TextRendererSharedPtr createLineNumberTextRenderer(size_t lineNumber,
                                                                        glm::ivec2 fontSize) {
    rendell_text::TextRendererSharedPtr result = rendell_text::makeTextRenderer();
    rendell_text::TextLayoutSharedPtr textLayout =
        createTextLayout(std::to_wstring(lineNumber), fontSize);
        ...
        if (_lineCount > value) {
            ...
            for (size_t i = 0; i < value - _lineCount; i++) {
                ...
                const rendell_text::TextRendererSharedPtr textRenderer =
                    createLineNumberTextRenderer(index + 1, _fontSize);
