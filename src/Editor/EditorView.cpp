...
void EditorView::addDocument(const std::wstring& name, const std::wstring& text)
{
	...
	if (!_documents.empty())
	{
		rendell_ui::TextWidgetSharedPtr textWidget = _documents.begin()->second;
		_documents.clear();
	...
	rendell_ui::TextWidgetSharedPtr document = createTextEdit(_rootWidget, text);
...
rendell_ui::TextWidgetSharedPtr EditorView::createTextEdit(rendell_ui::WidgetWeakPtr parent, const std::wstring& text) const
{
	rendell_ui::TextWidgetSharedPtr result = rendell_ui::createTextWidget(parent);
